...
public class PlayerState {
    ...
    public boolean hasRepeatedRotationPattern() {
        ...
        Float[] yawArray = yawHistory.toArray(new Float[0]);
    ...
    /**
     * Checks for suspicious yaw patterns.
     *
     * @return true if a suspicious pattern is detected, false otherwise
     */
    public boolean checkYawPattern() {
        if (yawHistory.size() < YAW_HISTORY_SIZE) {
            return false;
        }

        Float[] yawValues = yawHistory.toArray(new Float[0]);
        int identicalCount = 0;
        int alternatingCount = 0;

        for (int i = 1; i < yawValues.length; i++) {
            if (Math.abs(yawValues[i] - yawValues[i - 1]) < 1.0) {
                identicalCount++;
            }
            if (i > 1 && Math.abs(yawValues[i] - yawValues[i - 2]) < 1.0) {
                alternatingCount++;
            }
        }

        if (identicalCount > 7 || alternatingCount > 7) {
            yawViolations++;
            return true;
        }
        return false;
