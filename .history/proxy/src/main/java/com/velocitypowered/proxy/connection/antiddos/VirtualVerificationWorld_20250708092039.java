...
import com.velocitypowered.proxy.util.DimensionInfo;
...
import net.kyori.adventure.util.Pair;
...
public class VirtualVerificationWorld {
    ...
    private static final Logger logger = LogManager.getLogger(VirtualVerificationWorld.class);
    ...
    private void sendJoinGamePacket(ConnectedPlayer player, VirtualPlayer virtualPlayer) {
        try {
            ...
            joinGame.setPartialHashedSeed(ThreadLocalRandom.current().nextLong());
            ...
            joinGame.setShowRespawnScreen(true);
            joinGame.setDoLimitedCrafting(false);
            
            // Set up level names and dimension info
            joinGame.setLevelNames(ImmutableSet.of(WORLD_NAME));
            ...
            // Create basic registry data for virtual world
            joinGame.setRegistry(createVirtualWorldRegistry());
