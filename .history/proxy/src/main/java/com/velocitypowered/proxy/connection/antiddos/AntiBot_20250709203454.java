...
public class AntiBot {
  ...
  public void onPlayerLogin(LoginEvent event) {
    ...
    logger.debug("Player {} verification status - Verified: {}, Exempt: {}, Should verify: {}", 
    ...
    if (initialTargetServer.isPresent()) {
      logger.debug("Determined initial target server for {}: {}", 
          username, initialTargetServer.get().getServerInfo().getName());
    } else {
      logger.warn("No initial target server found for player {}", username);
    }
    
    // If mini-world check is enabled, reroute to virtual verification world
    if (shouldVerify) {
      logger.info("Player {} requires verification, redirecting to virtual verification world", username);
      
      // Store the original destination for later
      initialTargetServer.ifPresent(server -> {
        originalDestinations.put(playerId, server);
        logger.debug("Stored original destination for {}: {}", 
      ...
      if (player instanceof ConnectedPlayer) {
        ...
        if (enteredVirtualWorld) {
          logger.info("Player {} successfully entered virtual verification world", username);
          ...
          logger.debug("Created verification session for player {}", username);
        } else {
          logger.error("Failed to enter player {} into virtual verification world", username);
          // Fall back to bypass verification
          verifiedPlayers.add(playerId);
          logger.warn("Player {} verification bypassed due to virtual world error", username);
        }
      }
    } else {
      if (isVerified) {
        logger.debug("Player {} already verified, skipping verification", username);
      } else if (isExempt) {
        logger.debug("Player {} from exempt IP {}, skipping verification", username, address.getHostAddress());
      } else {
        logger.debug("Mini-world verification disabled, marking {} as verified", username);
      }
      
      // Mark as verified immediately if we're not doing the check
      if (!verifiedPlayers.contains(playerId)) {
        verifiedPlayers.add(playerId);
        logger.debug("Marked player {} as verified", username);
      }
    }
  }
  ...
  public void onPlayerDisconnect(DisconnectEvent event) {
    ...
    logger.debug("Player {} ({}) disconnecting from {}", username, playerId, hostAddress);
    ...
    if (wasInVirtualWorld) {
      logger.info("Player {} disconnected from virtual verification world", username);
    ...
    if (wasInVerification) {
      ...
      logger.info("Player {} disconnected during verification", username);
      logger.debug("Verification session details:");
      logger.debug("  Duration: {} ms", sessionDuration);
      logger.debug("  Completed: {}", sessionCompleted);
      logger.debug("  Movement count: {}", session.movementCount);
      logger.debug("  Interaction count: {}", session.interactionCount);
      logger.debug("  Distance moved: {:.2f}", session.getDistanceMoved());
      
      if (!sessionCompleted) {
        logger.warn("Player {} left verification early (potential bot behavior)", username);
      }
    } else if (wasVerified) {
      logger.debug("Verified player {} disconnected", username);
    } else if (wasSuspicious) {
      logger.info("Suspicious player {} disconnected", username);
    }
    ...
    if (connections != null) {
      ...
      logger.debug("IP {} now has {} active connections", hostAddress, newCount);
      if (newCount <= 0) {
        connectionsByIp.remove(address);
        logger.debug("Removed IP {} from connection tracking", hostAddress);
      }
    }
    ...
    logger.debug("Cleaned up all tracking data for player {}", username);
  }
  ...
  private void handleVirtualWorldConnection(Player player, RegisteredServer connectedServer) {
    ...
    logger.debug("Player {} ({}) in virtual verification world, attempting to connect to {}", 
        player.getUsername(), playerId, connectedServer.getServerInfo().getName());
    
    // Check if the player is still in verification
    MiniWorldSession session = miniWorldSessions.get(playerId);
    if (session == null) {
      logger.warn("Player {} has no verification session, bypassing verification", 
          player.getUsername());
      verifiedPlayers.add(playerId);
      virtualWorld.exitVerificationWorld((ConnectedPlayer) player);
      return;
    ...
    if (session.isCompleted() && session.isCheckPassed()) {
      logger.info("Player {} verification complete, transferring to target server", 
          player.getUsername());
      completeVerification(player, connectedServer);
    } else if (session.isCompleted() && !session.isCheckPassed()) {
      logger.warn("Player {} failed verification, denying connection", 
          player.getUsername());
      // Handle failed verification
      handleFailedVerification(player);
    } else {
      // Verification still in progress, keep player in virtual world
      logger.debug("Player {} verification still in progress", player.getUsername());
      ...
      logger.debug("Setting up verification timeout of {} seconds for player {}",
          config.getMiniWorldDuration(), player.getUsername());
      
      // Schedule the verification timeout
      server.getScheduler().buildTask(plugin, () -> {
        logger.debug("Verification timeout triggered for player {}", player.getUsername());
        ...
        if (player.isActive() && miniWorldSessions.containsKey(playerId) && 
          ...
          logger.debug("Processing verification results for player {}", player.getUsername());
          ...
          logger.debug("Player {} verification analysis:", player.getUsername());
          logger.debug("  Movement count: {}", currentSession.movementCount);
          logger.debug("  Interaction count: {}", currentSession.interactionCount);
          logger.debug("  Distance moved: {:.2f}", currentSession.getDistanceMoved());
          logger.debug("  Total path distance: {:.2f}", currentSession.getTotalPathDistance());
          logger.debug("  Movement complexity: {:.2f}", currentSession.getMovementComplexity());
          logger.debug("  Has jumped: {}", currentSession.hasJumped);
          logger.debug("  Has crouched: {}", currentSession.hasCrouched);
          logger.debug("  Has interacted: {}", currentSession.hasInteracted);
          logger.debug("  Natural timing: {}", currentSession.hasNaturalTiming());
          logger.debug("  Session duration: {} ms", 
              System.currentTimeMillis() - currentSession.startTime);
          logger.debug("  RESULT: {}", passed ? "PASSED" : "FAILED");
          
          // Mark session as complete
          currentSession.complete(passed);
          
          if (passed) {
            logger.info("Player {} PASSED verification check", player.getUsername());
            verifiedPlayers.add(playerId);
            transferToTargetServer(player);
          } else {
            logger.info("Player {} FAILED verification check", player.getUsername());
            if (config.isKickEnabled()) {
              logger.debug("Kicking player {} due to failed verification", player.getUsername());
              player.disconnect(Component.text(config.getKickMessage()));
            } else {
              // Still transfer, but mark as suspicious
              logger.debug("Marking player {} as suspicious and transferring", player.getUsername());
              suspiciousPlayers.add(playerId);
              transferToTargetServer(player);
            }
          }
        } else {
          logger.debug("Player {} no longer active or session already completed", 
              player.getUsername());
        }
  ...
  private void completeVerification(Player player, RegisteredServer targetServer) {
    ...
    // Transfer to the original or default server
    player.createConnectionRequest(targetServer).fireAndForget();
    
    logger.info("Player {} passed verification and is being sent to {}", 
  ...
  private void handleFailedVerification(Player player) {
    if (config.isKickOnVerificationFail()) {
      player.disconnect(Component.text(config.getKickMessage()));
      logger.info("Kicked player {} for failing verification", player.getUsername());
    } else {
      // Mark as suspicious and let them connect to a fallback server
      suspiciousPlayers.add(playerId);
      
      RegisteredServer fallbackServer = server.getServer(MINIWORLD_FALLBACK_SERVER)
          .orElse(server.getAllServers().stream().findFirst()
          .orElse(null));
      
      if (fallbackServer != null) {
        player.createConnectionRequest(fallbackServer).fireAndForget();
        logger.warn("Player {} failed verification, sent to fallback server {}", 
            player.getUsername(), fallbackServer.getServerInfo().getName());
      } else {
        player.disconnect(Component.text("Could not connect to a fallback server."));
        logger.error("Player {} failed verification, but no fallback server is configured!", 
            player.getUsername());
      }
    }
  }
  ...
  public void reportStatus() {
    ...
    logger.info("- Active: {}", enabled);
    logger.info("- Players being tracked: {}", playerStates.size());
    logger.info("- Verified players: {}", verifiedPlayers.size());
    logger.info("- Suspicious players: {}", suspiciousPlayers.size());
    logger.info("- Active mini-world sessions: {}", miniWorldSessions.size());
    logger.info("- Connections by IP count: {}", connectionsByIp.size());
    
    // Report configuration
    if (enabled) {
      logger.info("- Configuration:");
      logger.info("  - Gravity checks: {}", config.isGravityCheckEnabled());
      logger.info("  - Hitbox checks: {}", config.isHitboxCheckEnabled());
      logger.info("  - Rotation checks: {}", config.isYawCheckEnabled());
      logger.info("  - Client brand checks: {}", config.isClientBrandCheckEnabled());
      logger.info("  - Mini-world checks: {}", config.isMiniWorldCheckEnabled());
      logger.info("  - Check only first join: {}", config.isCheckOnlyFirstJoin());
    }
    
    // Report any active mini-world sessions
    if (!miniWorldSessions.isEmpty()) {
      logger.info("- Active mini-world sessions:");
      miniWorldSessions.forEach((id, session) -> {
        logger.info("  - Player: {}, Time: {}/{} sec, Movements: {}, Completed: {}, Passed: {}", 
  ...
  public void markPlayerAsVerified(UUID playerId) {
    ...
    if (session != null) {
      ...
      logger.info("Player {} marked as verified by virtual world", session.username);
    }
  }
  ...
  private void completeVerification(Player player, RegisteredServer targetServer) {
    ...
    logger.info("Player {} successfully completed verification.", username);
  ...
  private void handleFailedVerification(Player player) {
    ...
    if (config.isKickOnVerificationFail()) {
      logger.info("Kicking player {} for failing verification", username);
      player.disconnect(Component.text(config.getKickMessage()));
    } else {
      logger.warn("Player {} failed verification, denying connection",
          username);
  ...
  private void transferToTargetServer(Player player, RegisteredServer targetServer) {
    ...
    logger.info("Transferring player {} to server {}.",
